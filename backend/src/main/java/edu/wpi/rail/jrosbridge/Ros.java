...
public class Ros {
	...
	private static Logger LOGGER = Logger.getLogger(Ros.class);
	/**
	...
	// private Object topicCallbacskLock = new Object();

	// keeps track of callback functions for a given topic
	private final ConcurrentHashMap<String, CopyOnWriteArrayList<TopicCallback>> topicCallbacks;
	...
	private final CopyOnWriteArrayList<RosHandler> handlers;
	...
	/**
	 * Create a connection to ROS with the given hostname and default port. A call
	 * to connect must be made to establish a connection. By default, WebSockets is
	...
	public Ros(String hostname, int port, JRosbridge.WebSocketType protocol) {
		...
		this.session = null;
		this.idCounter = 0;
		this.topicCallbacks = new ConcurrentHashMap<String, CopyOnWriteArrayList<TopicCallback>>();
		this.serviceCallbacks = new HashMap<String, ServiceCallback>();
		this.callServiceCallbacks = new HashMap<String, CallServiceCallback>();
		this.handlers = new CopyOnWriteArrayList<RosHandler>();
	}
	...
	/**
	 * Add a handler to this connection. This handler is called when the associated
	 * events occur.
	 * 
	 ...
	 * @param handler The handler to add.
	 */
	public void removeRosHandler(RosHandler handler) {
	 ...
	 * error output stream.
	 * 
	 * @return Returns true if the connection was established successfully and false
	 *         otherwise.
	 */
	public boolean connect() {
		try {
			...
			ContainerProvider.getWebSocketContainer().setAsyncSendTimeout(60000);
			return true;
		} catch (DeploymentException | URISyntaxException | IOException e) {
			// failed connection, return false
			LOGGER.error("[ERROR]: Could not create WebSocket: " + e.getMessage());
			return false;
		}
	}
	 ...
	 * output stream.
	 * 
	 * @return Returns true if the disconnection was successful and false otherwise.
	 */
	public boolean disconnect() {
		if (this.isConnected()) {
			try {
				...
				LOGGER.error("[ERROR]: Could not disconnect: " + e.getMessage());
			}
		}
	 ...
	 * 
	 * @param session The session associated with the disconnection.
	 */
	@OnClose
	 ...
	 * 
	 * @param message The incoming JSON message from rosbridge.
	 */
	@OnMessage
	public void onMessage(String message) {
		try {
			...
			if (op.equals(JRosbridge.OP_CODE_PNG)) {
				...
				// read the RGB data
				int[] rawData = null;
				rawData = imageData.getPixels(0, 0, imageData.getWidth(), imageData.getHeight(), rawData);
	...
	private void handleMessage(JsonObject jsonObject) {
		...
		// System.out.println("handleMessage: " + jsonObject.toString());
		...
		if (op.equals(JRosbridge.OP_CODE_PUBLISH)) {
			...
			//synchronized (topicCallbacskLock) {
				CopyOnWriteArrayList<TopicCallback> callbacks = topicCallbacks.get(topic);
				if (callbacks != null) {
			...
			if (cb != null) {
				...
				boolean success = jsonObject.containsKey(JRosbridge.FIELD_RESULT)
						? jsonObject.getBoolean(JRosbridge.FIELD_RESULT)
						: true;
				// get the response
				// if(jsonObject.i)
				...
				try {
					values = jsonObject.getJsonObject(JRosbridge.FIELD_VALUES);
				} catch (ClassCastException cce) {
					String errorMessage = jsonObject.getString(JRosbridge.FIELD_VALUES);
			...
			if (cb != null) {
				// get the response
				JsonObject args = jsonObject.getJsonObject(JRosbridge.FIELD_ARGS);
	...
	public synchronized boolean send(JsonObject jsonObject) {
		...
		if (this.isConnected()) {
			try {
				...
				LOGGER.error("[ERROR]: Could not send message: " + e.getMessage());
			}
		}
		// message send failed
		return false;
	 ...
	 */
	public void authenticate(String mac, String client, String dest, String rand, int t, String level, int end) {
		// build and send the rosbridge call
		JsonObject call = Json.createObjectBuilder().add(JRosbridge.FIELD_OP, JRosbridge.OP_CODE_AUTH)
				.add(JRosbridge.FIELD_MAC, mac).add(JRosbridge.FIELD_CLIENT, client)
				.add(JRosbridge.FIELD_DESTINATION, dest).add(JRosbridge.FIELD_RAND, rand).add(JRosbridge.FIELD_TIME, t)
				.add(JRosbridge.FIELD_LEVEL, level).add(JRosbridge.FIELD_END_TIME, end).build();
		this.send(call);
	}
	...
	public void registerTopicCallback(String topic, TopicCallback cb) {
		...
		//synchronized (topicCallbacskLock) {
			...
			if (!this.topicCallbacks.containsKey(topic)) {
				this.topicCallbacks.put(topic, new CopyOnWriteArrayList<TopicCallback>());
			}
	...
	public void deregisterTopicCallback(String topic, TopicCallback cb) {
		//synchronized (topicCallbacskLock) {
			...
			if (this.topicCallbacks.containsKey(topic)) {
				...
				CopyOnWriteArrayList<TopicCallback> callbacks = this.topicCallbacks.get(topic);
	 ...
	 * @param cb            The callback that will be called when a service response
	 *                      comes back for the associated request.
	 */
	 ...
	 * @param cb          The callback that will be called when a service request
	 *                    comes in for the associated request.
	 */
	...
	public String getTopicType(String topicName) {
		...
		Service messageDetailService = new Service(this, "/rosapi/topic_type", "rosapi/TopicTypeResponse");
